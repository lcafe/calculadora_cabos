 ...
 */
function correnteProjeto(fases, P, V, FP, rendimento, tipoCarga, instalacaoIndustrial, fatorDeDemanda) {
    if ([P, V, FP, rendimento, fatorDeDemanda].some(v => typeof v !== "number" || v < 0)) {
    ...
    function correnteMonofasico(P, Vfn, FP, fatorDeDemanda) {
      if (Vfn * FP === 0) throw new Error("Vfn e FP não podem ser zero.");
      return ((P * fatorDeDemanda) / (Vfn * FP));
    }

    function correnteBifasico(P, Vff, FP, fatorDeDemanda) {
      if (Vff * FP === 0) throw new Error("Vff e FP não podem ser zero.");
      return ((P * fatorDeDemanda) / (Vff * FP));
    }

    function correnteTrifasico(P, Vff, FP, fatorDeDemanda) {
      if (Vff * FP === 0) throw new Error("Vff e FP não podem ser zero.");
      return ((P * fatorDeDemanda) / (Math.sqrt(3) * Vff * FP));
    }

    function correnteMotorica(P, Vff, FP, rendimento, fatorDeDemanda) {
      if (Vff * FP === 0) throw new Error("Vff e FP não podem ser zero.");
      if (rendimento === 0) throw new Error("Rendimento não pode ser zero.");
      return ((P * fatorDeDemanda) / (Math.sqrt(3) * Vff * FP * rendimento));
    }

  
    switch (fases) {
      case 1:
        return correnteMonofasico(P, V, FP, fatorDeDemanda);
      case 2:
        return correnteBifasico(P, V, FP, fatorDeDemanda);
      case 3:
        if(tipoCarga === "motorica") { 
          return correnteMotorica(P, V, FP, rendimento, fatorDeDemanda);	
        } else {
        return correnteTrifasico(P, V, FP, fatorDeDemanda);
      }
      default:
        throw new Error(
